...
public class FoodSearchBar extends GridPane {
	...
	public FoodSearchBar() {
		isUsda = new SimpleBooleanProperty();
		isUsda.setValue(true);
		hbox1 = new HBox();
		hbox2 = new HBox();
		vbox = new VBox();
		...
		this.searchOptions.setOnAction(e -> {
			if	(isUsda.getValue())
				this.foodGroups.setDisable(true);
			else
				this.foodGroups.setDisable(false);
			this.isUsda.setValue(!this.isUsda.getValue());
			this.usdaGraphic.setVisible(isUsda.getValue());
		});
